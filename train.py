...
print("models.")
if args.ema > 0:
    from torch_ema import ExponentialMovingAverage #指数移动平均（EMA）提高模型性能和泛化能力

if args.wandb:
    import wandb #WandB进行实验跟踪和可视化
...
def train(model, train_loader, optimizer, epoch, scheduler, mixup = False, mm = False):
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if mixup and args.mm: # mixup or manifold_mixup
            ...
            output, features = model_output[0], model_output[1]
            if args.rotations:
                output, output_rot = output
...
if __name__ == '__main__':
    ...
    def create_model(args):
        if args.model.lower() == "multigrainNet":
            return multigrainNet.multigrain(args.feature_maps, input_shape, num_classes, few_shot,args.rotations).to(args.device)
        if args.model.lower() == "res2net":
            return res2net.Res2Net12(args.feature_maps, input_shape, num_classes, few_shot,args.rotations).to(args.device)
